...
func build_tree() -> void:
	if tabs:
		match tabs.current_tab:
			0:
				project.build_tree(todo_items, ignore_paths, patterns, plugin.cased_patterns, _sort_alphabetical, full_path, _group_by_priority)
				create_config_file()
			1:
...
func create_config_file() -> void:
	...
	config.set_value("scripts", "group_by_priority", _group_by_priority)
...
func load_config() -> void:
	...
	if config.load("res://addons/Todo_Manager/todo.cfg") == OK:
		...
		_group_by_priority = config.get_value("scripts", "group_by_priority", DEFAULT_GROUP)
...
func _on_GroupByPriority_toggled(button_pressed: bool) -> void:
	_group_by_priority = button_pressed
	plugin.rescan_files(true)
